...
\begin{tikzpicture}[@O]
    ...
    \hlbehindcode{IntParse}{@>}
\end{tikzpicture}
\columns[onlytextwidth,c]
\column{.475\linewidth}
\begin{plainjava}
public static void main(String[] args) {
    !*\Snode{IntParse}*!int zahl = Integer.parseInt(args[0]);!*\Snode{IntParse@}*!
   if (zahl < 0) {
...
\begin{tikzpicture}[@O,K/.style={above,gray,font=\footnotesize\sffamily}]
    \draw[lightgray,-Kite] (@>) to[out=5,in=165,edge node={node[K] {Ein allgemeinerer Eingabeparameter}}] ([yshift=1mm]IntParseNew.north);
\end{tikzpicture}
\end{frame}
