...
\begin{tikzpicture}[@O]
    ...
    \end{tabular}\medskip\\\centerline{\Large \textbf{Sichtbarkeits}modifikatoren}};}
    \onslide<4->{\node[right,T] at(classline.east) {Objekte leben, wie mind. eine Referenz auf sie gültig ist.};}
    \onslide<5->{
        \draw[decoration=brace,decorate,lightgray] ([xshift=2mm,yshift=1.5mm]@age.north east) coordinate (@) to[edge node={node[right=1mm,T,align=left] {Die Attribute besitzen die selbe \textup{Gültigkeit},\\wie das Objekt, dem sie gehören.}}] ([yshift=-1.5mm]@name.south east-|@);
    }
    \onslide<6->{
