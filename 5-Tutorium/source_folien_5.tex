...
\scope[shift=(current page.east),xshift=-2.35cm,yshift=2.5cm,scale=.9]
    ...
    \onslide<17->{\path(base.north)--(base@.north) coordinate[pos=.5] (@);
        \draw[Kite-,codeouthl] ([xshift=3mm]@.north) to[bend right] ++(-.5,.25) node[left,T] {Für Kleinbuchstaben ändert sich nur die Normalisierung.};
    }
    \onslide<19->{
        \draw[Kite-,codeouthl] (magic-num.south) to[bend right] ++(.5,-.5) node[right,align=left,T] {Magic-Number (Alphabeeet)};
    }
    \onslide<20->{
        \draw[Kite-,codeouthl] (floorMod.south) to[bend left] ++(-.5,-.35) node[left,T] {Javas Modulo beachtet das Vorzeichen!};
    }
\endscope
\end{tikzpicture}
\end{frame}

\begin{frame}[fragile]{Ein finales Beispiel}
\SetupLstHl
\begin{itemize}[<+(1)->]
...
\end{itemize}
\begin{tikzpicture}[@O]
    \onslide<8->{\node[T,right] at(newarr.east) {\T{System.out.println(char[])}};}
...
\begin{frame}{Refactoring}
 \begin{itemize}[<+(1)->]
    \itemsep8pt
     ...
     \item Das Stichwort lautet \textbf{Refactoring}
        \begin{itemize}
            \itemsep3pt
            \item Code lässt sich immer aufräumen und leserlicher, testbarer,~\ldots\ gestalten
            \item Die eigentliche Implementierung nimmt relativ wenig Zeit ein~\cite{10.5555/2820282.2820289} \infoblock{Die meiste Zeit \say{stirbt} für das Verstehen von bestehendem Code (je nach Größe rund \qty{70}\percent), aber auch Testen und Anforderungsdefinitionen bekommen einen Teil.}
            \item Selbst bei kleinen Projekten \info{ungefähr \num{1000} Zeilen} nimmt die Entwicklung nur $\approx$\qty{50}\percent\ Zeit ein.
        \end{itemize}
     \item Nehmt euch doch am Ende nochmal 20~Minuten und versucht euren Code zu verbessern
 \end{itemize}
\end{frame}
}
