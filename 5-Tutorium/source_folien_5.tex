...
\begin{plainjava}[lineskip=4pt]%,morekeywords={[3]{Penguin}}]
...
\begin{tikzpicture}[@O]
    ...
    \onslide<6->{
        \fill[rounded corners=1.6pt,pingu@green,opacity=\hlopa] ([xshift=1mm,yshift=-5pt]@CLASS) rectangle ([xshift=1mm-3.2pt,yshift=5pt]@CLASS@);
...
\gets{18}{w.canWaddleOn}{null} \info{kein \T{new}!}\\
...
!*\onslide<9->*!      Penguin[] pengus = { !*\onslide<9->*!new Penguin("Hugo", 3),
!*\onslide<9->*!        !*\onslide<9->*!new Penguin("Sophie", 7) !*\onslide<8->*!};
...
\begin{tikzpicture}[@O]
    ...
    \onslide<22-25|handout:0>{\node[ca@CommentNode@out] at(ca@@-4@end) {\T{\strut sum = 10}};}
    \onslide<26->{\node[below left,T,align=right,yshift=-1.5cm] at(current page.north east) {Hier ist Platz für Heap und Stack.\\Oh, schon 2~Uhr.};}
\end{tikzpicture}
\end{frame}
...
\begin{itemize}[<+(1)->]
    ...
    \item Java unterscheidet vier verschiedene Sichtbarkeitsbereiche \info{von denen drei bisher relevant sind}\begin{itemize}
        ...
        \item \bjava{public}: überall sichtbar ist
        \item nichts: entspricht \say{package-private}
        \item \bjava{private}: nur innerhalb der Klasse
    \end{itemize}
