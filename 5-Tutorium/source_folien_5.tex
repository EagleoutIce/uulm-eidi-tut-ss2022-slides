...
\begin{itemize}[<+(1)->]
    \item Wir sollen mehrere Quersummen berechnen. Wir haben schon ein Programm für eine:\vfill\lstfs{8}
\columns[onlytextwidth,c]
\column{.475\linewidth}
\begin{plainjava}
public static void main(String[] args) {
   int zahl = Integer.parseInt(args[0]);!*\Snode{IntParse}*!
   if (zahl < 0) {
...
\begin{tikzpicture}[@O]
    ...
    \iftikzmark{IntParseNew}{% save@pt@
        \edef\Want{\csname save@pt@\tikzmark@pp@name{IntParseNew}\endcsname}
        \draw (\Want.east) circle[radius=1mm];
    }{}
    % \hlcode{IntParseNew}{@}
\end{tikzpicture}
\begin{plainjava}
public static int quersumme(!*\tikzmarknode{IntParseNew}{\sbasic{\skB{int} zahl}}*!){
    if(zahl < 0){
...
\endcolumns
\end{itemize}
\begin{tikzpicture}[@O,K/.style={above,gray,font=\footnotesize\sffamily}]
    \draw[lightgray,-Kite] (IntParse) to[out=5,in=165,edge node={node[K] {Ein allgemeinerer Eingabeparameter}}] ([yshift=1mm]IntParseNew.north);
\end{tikzpicture}
\end{frame}
