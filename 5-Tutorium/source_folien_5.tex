...
\SetupLstHl
\begin{itemize}[<+(1)->]
    \item Nun gilt es noch aufzurufen:
\begin{plainjava}
!*\onslide<3->*!|ihl|public class CharRotation {|ihl|
!*\onslide<4->*!    public static void main(String[] args) {
!*\onslide<5->*!        char[] arr = { 'a', 'Z' };
!*\onslide<6->*!        rotiereCharacterArray(arr);
!*\onslide<7->*!        System.out.println(arr);!*\Snode{newarr}*!
!*\onslide<4->*!    }
!*\onslide<3->*!|ihl|}|ihl|
\end{plainjava}
\end{itemize}
...
\begin{enumerate}
    ...
    \item<10-> \task{Geben Sie eine Instanz der Klasse \T{Warenkorb} an.}
\end{enumerate}\vfill
\begin{center}
    \scalebox{.65}{\begin{tikzpicture}
\tikzumlset{fill class=btdm@background}
...
\onslide<7->{fügeArtikelHinzu(Artikel: \solGet{keywordC}{Artikel}): void}\\
\onslide<8->{entferneArtikel(index: int): boolean}\\
\onslide<9->{berechneGesamtpreis(): float}\\
}}

\onslide<11->{\umlclass[x=10,y=0,rounded corners=4pt]{Objekt: Mein Warenkorb}{
    \onslide<12->{Kundennummer: 744392}\\
    \onslide<13->{Artikelliste: [\textit{Mein Hemd}]}
}{
    \onslide<14->{fügeArtikelHinzu(Artikel: \solGet{keywordC}{Artikel}): void}\\
    \onslide<14->{entferneArtikel(index: int): Artikel}\\
    \onslide<14->{berechneGesamtpreis(): float}\\
}}
\end{tikzpicture}}
\end{center}
\begin{itemize}
    \item Es ist nicht unbedingt notwendig, mit \textit{Main Hemd}, ein Objekt zu referenzieren \begin{itemize}
        \itemsep2.5pt
        \item Je nach Problem, reicht vielleicht auch einfach die Artikelnummer, da sie eindeutig ist.
        \item Man kann das Objekt auch an Ort und Stelle \say{schreiben}\infoblock{Dann mit meist in \link{https://www.json.org/json-de.html}{JSON}-(ähnlicher)-Syntax \T{\{Name: \dq Hemd \dq, Preis: 42.99f, \textcolor{codeouthl}{\ldots}\}}}
    \end{itemize}
\end{itemize}
\end{frame}
...
!*\onslide<3->*!!*\CodeFileMarkerAttach{Penguin.java}*!
...
!*\onslide<8->*!   public int !*\Snode{getAge}*!getAge!*\Snode{getAge@}*!() { return age; }!*\Snode{ageStart}*!
!*\onslide<9->*!   public String toString() {
!*\onslide<10->*!      return name + " watschelt seit " + age + " Jahr(en)";!*\Snode{longest}*!
...
\begin{tikzpicture}[@O]
    ...
    \onslide<20->{
        ...
        \onslide<21->{
            ...
            \node[below left,xshift=9mm,yshift=1.25mm, T,align=right] (@) at(@.south east) {Der leere default-Konstruktor existiert nur genau dann, wenn kein expliziter existiert. \\Hier existiert \T{Penguin(String, int)}.};
        }
    }
    \onslide<23->{
    ...
    % ist \textit{keine Methode} und
\end{tikzpicture}
% TODO: getAge and stuff
% TODO: highlight others
\end{frame}

\begin{frame}[fragile,c]{Pinguineeee --- Gültigkeit \& Sichtbarkeit}
\lstfs{9} TODO Tabelle und so
\begin{plainjava}[lineskip=4.5pt]
!*\onslide<3->*!!*\CodeFileMarker{Penguin.java}*!
!*\onslide<2->*!public class Penguin {
!*\onslide<2->*!   private final int age;
!*\onslide<2->*!   final String name;
!*\onslide<2->*!
!*\onslide<2->*!   public Penguin(String name, int age) {
!*\onslide<2->*!       this.name = name;
!*\onslide<2->*!       this.age = age;
!*\onslide<2->*!   }
!*\onslide<2->*!
!*\onslide<2->*!   public int getAge() { return age; }
!*\onslide<2->*!   public String toString() {
!*\onslide<2->*!      return name + " watschelt seit " + age + " Jahr(en)";
!*\onslide<2->*!   }
!*\onslide<2->*!}
\end{plainjava}
\end{frame}
...
TODO: auch default werte
TODO: Ctor keine Methode?
