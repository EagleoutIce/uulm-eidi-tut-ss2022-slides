...
\forestset{K/.style={for tree={inner sep=4pt,block,minimum width=2em,outer sep=1pt,execute at begin node={\strut},grow=north,s sep=1.25em}}}
...
\end{forest}}\onslide<4->{\qquad$\equiv$\qquad\raisebox{-.5\height}{\begin{forest} K
    % TODO: do the 0 6 false thingy first, and do not evaluate the b = 3 stuff in the then case, be consistent!
    [?,label=left:if[\paletteA{\bfseries?},label=right:ggT,edge label={node[midway,right]{\footnotesize else}}[3,label=above:b][\only<9->{0}[\only<8->{3},label=above:b][\only<8->{6},label=left:a]]][\only<7->{\color{codeouthl}3},label=above:b, edge label={node[midway,left,sloped]{\footnotesize then}}][\only<6->{false},label=left:{=}, edge label={node[midway,left]{\footnotesize when}}[\only<5->{3},label=right:b][0]]]
    \node[above right,yshift=4mm] at(current bounding box.north west) {Inkarnation \(1\): \(a = 6\), \(b = 3\)};
