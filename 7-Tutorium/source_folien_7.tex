...
\begin{enumerate}
    \itemsep7pt
    \setcounter{enumi}{-1}
    \item<2-|handout:1-> Initial ist \(\text{min = 0}\) und \(\text{max = 7}\). Wir springen in die Mitte: \(\text{mid} = \floor{(\text{min} + \text{max})/2}\). \onslide<3->{\infoblock{Hierbei runden wir im Gleichheitsfall ab. Das ist an sich willkürlich, hier aber von uns festgesetzt.}}
...
\forestset{K/.style={for tree={inner sep=4pt,block,minimum width=2em,outer sep=1pt,execute at begin node={\strut},grow=north,s sep=1.3em,l sep=1.5em}}}
\tikzset{every label/.style={gray},every path/.append style={line cap=round}}
...
\end{forest}}\onslide<4->{\quad$\equiv$\quad\raisebox{-.5\height}{\begin{forest} K
    ...
    [\only<11->{?},label=left:if[\only<11->{\paletteA{\bfseries?}},label=right:ggT,edge label={node[midway,right]{\footnotesize else}}[\only<10->{3},label=above:b][\only<9->{0}[\only<8->{3},label=right:b][\only<8->{6},label=left:a]]][\only<7->{\color{codeouthl}3},label=above:b, edge label={node[pos=.55,left,align=right]{\footnotesize th\rlap{-}\\[-2.5mm]\footnotesize en}}][\phantom{false}\only<6->{\llap{false}},label=left:{=}, edge label={node[midway,left]{\footnotesize when}}[\only<5->{3},label=right:b][0]]]
