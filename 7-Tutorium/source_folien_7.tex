...
\begin{itemize}[<+(1)->]
    \item Für die \(\O\)-Notation gilt \(T(n) \in \O(f(n))\), wenn \(f(n)\) eine obere Schranke von \(T(n)\) ist, also: \onslide<3->{\[T(n) \in \O(f(n))\pause{}\iff \exists n_0 \in \mathbb{N}\: c \in \mathbb{R}^+\: \forall n \geq n_0: T(n) \leq c \cdot f(n).\vspace*{-\baselineskip}\]}
