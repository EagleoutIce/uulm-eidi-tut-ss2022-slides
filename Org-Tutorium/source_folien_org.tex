...
\null\hfill\begin{minipage}{.8\linewidth}
    \rule{0cm}{1.25cm}\llap{\color{gray}\onslide<2->{Theorie\quad}}\only<3->{\LoadOverview{1}{}}\hfill
    %\only<12->{\LoadOverview{1}{-active}}
    \onslide<10->{\LoadOverview{8}{}}\medskip\\
    \llap{\color{gray}\onslide<2->{Grundlagen\quad}}\onslide<4->{\LoadOverview{2}{}}\onslide<5->{\LoadOverview{3}{}}\onslide<6->{\LoadOverview{4}{}}\medskip\\
    \llap{\color{gray}\onslide<2->{Fortgeschritten\quad}}\hfill\onslide<7->{\LoadOverview{5}{}}\hfill\onslide<8->{\LoadOverview{6}{}}\onslide<9->{\LoadOverview{7}{}}\hfill\onslide<11->{\LoadOverview{9}{}}
\end{minipage}\par
\end{frame}
...
\begin{frame}[t]{Übungsblätter\hfill II}
    \begin{itemize}[<+(1)->]
        ...
        \item Im Falle einer Krankheit:\pause~Nachricht an Tutor\gend in mit zugehöriger Entschuldigung!\pause\infoblock{Eine schriftliche Entschuldigung, kein Attest oder privates Dokument. Extremfälle werden einzeln entschieden.}
    \end{itemize}
    \vfill
    \begin{center}
