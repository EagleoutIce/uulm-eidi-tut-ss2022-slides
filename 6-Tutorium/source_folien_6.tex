...
\end{enumerate}
\begin{tikzpicture}[@O]
    \onslide<6->{\draw[lightgray,-Kite] (@.south) to[out=270,in=180] ++(.5,-.33) node[T,below right,yshift=.7\baselineskip,align=left,text width=8.5cm] {\T{this} bezieht sich auf das Objekt auf dem die Methode aufgerufen wird. Statische Methoden werden aber auf der Klasse aufgerufen! \only<7->{Das gilt natürlich auch ohne das \T{this}, da es sich dann immernoch auf eine Instanzvariable bezieht.}};}
\end{tikzpicture}
\end{frame}
...
{\makeatletter\lhns@elemwidth3.66cm
% with parbox i try to keep fontsize (especially resets) out of tikz mechanisms
% custom -#mm for padding
\def\Object#1#2{\centering\footnotesize\parbox{\linewidth-2.5mm}{\centerline{\bjava{#1} \textcolor{gray}{(object)}}\relax\ignorespaces#2}}
...
\pgfmathsetmacro\lhns@render@heap@jitter{-abs(rand)*1mm-.85mm}% just to be save and keep it constant over anims!
...
\tikzset{ca@CommentNode/.append style={font=\sffamily\scriptsize\slshape,right=.5mm,yshift=-.5mm,scale=.75},ca@CommentNode@out/.append style={font=\sffamily\scriptsize\slshape}}% compact :)
...
/*\onslide<2->*/  }
/*\onslide<2->*/  public ValuePair getValuePair() {
/*\onslide<2->*/    return v;
/*\onslide<2->*/  }
/*\onslide<2->*/}/*\smallskip*/
...
/*\onslide<4->*/    String n1 = p.getName();
/*\onslide<4->*/    ValuePair xy1 = p.getValuePair();
/*\onslide<4->*/    System.out.println(n1 + " " + xy1.x + " " + xy1.y);
/*\onslide<4->*/    n1 = "Hugo";
/*\onslide<4->*/    String n2 = p.getName();
/*\onslide<4->*/    xy1.x = 33;
/*\onslide<4->*/    xy1.y = 44;
/*\onslide<4->*/    ValuePair xy2 = p.getValuePair();
/*\onslide<4->*/    System.out.println(n1 + " " + xy1.x + " " + xy1.y);
/*\onslide<4->*/  }
/*\onslide<4->*/}/*\onslide<1->*/
\end{minted}
...
\endcolumns
\onslide<6->{% just for the alignment
...
\only<10->{\heap{\Object{Person}{\bjava{String name}\\\bjava{ValuePair v}}}}
% TODO: genau genommen existieren ja auch die Literale aber psst
\only<11->{\heap{\bjava{\"Heinz\"}}}
\only<16->{\heap{\Object{ValuePair}{\only<-17>{\bjava{int x = ?}}\only<18->{\bjava{int x = 11}}\\\only<-17>{\bjava{int y = ?}}\only<18->{\bjava{int y = 22}}}}}
\renderheap
% TODO: eingehen auf initialisierung, instanziierung , zuweisung etc...

\only<7->{\stack{\textbf{main}}}
\only<8->{\stack{\bjava{Person p}}}
\only<10->{\stack{\textbf{\protect\downsize{\linewidth-4mm}{<ctor> Person}}}}
\only<11-14>{\stack{\bjava{String name}}}
\only<11-14>{\stack{\bjava{int x = 11}}}
\only<11-14>{\stack{\bjava{int y = 22}}}
\only<15->{\stack{\color{gray}{\protect\downsize{\linewidth-4mm}{\itshape\ldots name, x, y\ldots}}}}
\only<15->{\stack{\textbf{\protect\downsize{\linewidth-4mm}{<ctor> ValuePair}}}}
\only<17->{\stack{\bjava{int x = 11}}}
\only<17->{\stack{\bjava{int y = 22}}}
\renderstack

% TODO: only when coming back
% \only<10->{\draw[lhns] (stack-1.east) to[out=0,in=180] (heap-0.west);}
\pgfinterruptboundingbox
{% color group
    \only<11->{\only<15->{\color{gray}}\draw[lhns] (stack-3.east) to[out=0,in=180] (heap-1.west);}
}
\only<12->{\draw[lhns] ([xshift=-1.33cm,yshift=-1mm]heap-0.east) to[out=0,in=0,looseness=2] (heap-1.east);}
...
\begin{tikzpicture}[@O]% THATS NO PENGU :C The federation of penguin support would never lie!
    ...
    \onslide<6>{\node[below left,yshift=-1.25cm,xshift=-1mm,T,scale=.65] at(current page.north east) {Aus Platzgründen ignoriere ich, dass \T{args} auch auf Heap und Stack verwaltet wird!};}
\end{tikzpicture}}
\end{frame}
}
