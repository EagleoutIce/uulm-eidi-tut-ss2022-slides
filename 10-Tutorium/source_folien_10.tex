...
\end{minipage}}\quad\begin{minipage}{.25\linewidth}
    \begin{enumerate}
        \item \(\to 2\)
        \item \(\to 3 \to 4\)
        \item \(\to 1 \to 4\)
...
\begin{itemize}[<+(1)->]
    \itemsep6pt
    \item Je nach Kontext bietet sich die Adjazenzmatrix (links) oder die Adjazenzliste (rechts) mehr an
    \item Für die Durchläufe markiert \(S\) den Startknoten
    \item Auch hier hilft die \hyperlink{episode-trav-jmp}{Episode}
\end{itemize}
\end{frame}
