...
\begin{frame}[c]{Wääärbuuuung}
    \begin{center}
        ...
        \end{tikzpicture}}\qquad\fancyqr{https://github.com/EagleoutIce/Episode-Traversierung}
    \end{center}
\end{frame}}\fi
...
\endtaskblock
\end{onlyenv}
\begin{onlyenv}<handout:2|3->
\def\intaskfont{\scriptsize}
\begin{enumerate}[<+(1)->]
    \item \task{Nehmen Sie die Klassendefinition für \textattachfile{a2/Element.java}{Element}, die Sie bereits aus der Vorlesung kennen. Erweitern
   ...
   \T{NegativeArraySizeException} ausgelöst werden}
    \item \task{Die Methode \T{public int peek()} der Klasse RingBuffer soll den Wert an der Position zurückliefern, auf die
    \T{head} zeigt. Sollte der Buffer leer sein, soll eine \T{NoSuchElementException} ausgelöst werden.}
    \item \task{Die Methode \T{public void put(int value)} der Klasse RingBuffer soll den übergebenen Wert an der
    ...
    \T{head} immer auf das älteste Element im Buffer zeigt und so die FIFO Eigenschaft gewahrt bleibt. Ein Beispiel für einen vollen Buffer der Kapazität 3 in das der Wert 4 eingefügt werden soll}
    \item \task{Die Methode \T{public int remove()} der Klasse RingBuffer soll den Wert an der Position zurückliefern, auf
...
\begin{frame}[fragile]{Extend the Element}
\begin{enumerate}[<+(1)->]
    \item \task{Nehmen Sie die Klassendefinition für \textattachfile{a2/Element.java}{Element}, die Sie bereits aus der Vorlesung kennen. Erweitern
    Sie diese um eine private Referenz für den Vorgänger sowie passende getter und setter. Die Elemente sollen \bjava{int} halten.}
\end{enumerate}
...
|ihl|    private int value;|ihl|
|ihl|    private Element next;|ihl|
/*\onslide<5->*/    private Element previous;

|ihl|    public Element() { this.next = null;  /*\onslide<6->*/|ihl|this.previous = null;|ihl| /*\onslide<1->*/}|ihl|
|ihl|    public void setNextElement(Element nextElement) { this.next = nextElement; }|ihl|
|ihl|    public Element getNextElement() { return this.next; }|ihl|
/*\onslide<7->*/public void setPreviousElement(Element previousElement) {
/*\onslide<7->*/    this.previous = previousElement;
/*\onslide<7->*/}
/*\onslide<8->*/public Element getPreviousElement() {
/*\onslide<8->*/    return this.previous;
/*\onslide<8->*/}
|ihl|    public void setValue(int value) { this.value = value; }|ihl|
|ihl|    public int getValue() { return this.value; }|ihl|
|ihl|}|ihl|/*\onslide<1->*/
\end{minted}
\end{onlyenv}
\end{frame}
