...
\midrule
    \onslide<3->{\tikzmarknode{itstart}{Bubble}}      & \onslide<3->{\(\O(n^2)\)\textcolor{gray}{, \(\O(n)\)}}       & \onslide<4->{\(\O(n^2)\)} & \onslide<3->{Vertausche benachbarte El., solange unsortiert.} \\
    ...
    \onslide<9->{\tikzmarknode{rekstart}{Merge}}    & \onslide<8->{\(\O(n \log n)\)} & \onslide<8->{\(\O(n \log n)\)}  & \onslide<9->{Aufteilen bis einel., wiederholtes mergen sortierter Teill.}\\
    \onslide<10->{Quick}                             & \onslide<10->{\(\O(n \log n)\)} & \onslide<10->{\(\O(n^2)\)}    & \onslide<11->{Pivot $\to$ Ende, \(l\) sol. \(<\), \(r\) sol. \(\geq\). Treffen \(\to\) tausche Pivot.}\\
    \onslide<12->{\tikzmarknode{rekend}{Heap}}       & \onslide<12->{\(\O(n \log n)\)} & \onslide<12->{\(\O(n\log n)\)} & \onslide<13->{Baue Heap, entferne wiederholt Wurzel, heapify.}\\
\bottomrule
...
\begin{tikzpicture}[@O]
    \onslide<10->{\draw[decoration={brace,mirror},decorate] ([xshift=-1.25mm]itstart.north west) to[edge node={node[left=1mm] {\rotatebox{90}{iterativ}}}] ([xshift=-1.25mm]itend.south west);}
    \onslide<11->{\draw[decoration={brace,mirror},decorate] ([xshift=-1.25mm]rekstart.north west) to[edge node={node[left=1mm] {\rotatebox{90}{rekursiv}}}] ([xshift=-1.25mm]rekend.south west);}
\end{tikzpicture}
\end{frame}
...
\begin{itemize}
    ...
    \item<0-> Diese erlauben es, abstrakter Datentypen umsetzen:\smallskip
...
\begin{enumerate}[<+(1)->]
    \item<6-> \task{Nehmen Sie die Klassendefinition für \only<6->{\textattachfile{a2/Element.java}{Element}}, die Sie bereits aus der Vorlesung kennen. Erweitern
...
\begin{enumerate}[<+(1)->]
    \item \task{Nehmen Sie die Klassendefinition für \only<2->{\textattachfile{a2/Element.java}{Element}}, die Sie bereits aus der Vorlesung kennen. Erweitern
...
\setbox\pinguB=\hbox{\tikz{\pingu[sunglasses round,tie=pingu@black,tie dots,cloak=pingu@black,left wing hug,eyes angry]}}
\setbox\pinguC=\hbox{\tikz{\pingu[sunglasses,tie=pingu@black,tie dots,cloak=pingu@black,right wing hug,eyes angry]}}
\setbox\pinguD=\hbox{\tikz{
...
\outro{\vskip9mm\centering \onslide<2->{\scalebox{1.35}{\begin{tikzpicture}
    % \pingu[conical hat, blush,santa beard, cane right, cape=paletteA]
    [W/.style={rounded corners,draw,lightgray,line width=4pt,minimum width=1.5cm,minimum height=.65cm,scale=2,font=\sffamily\Large,text=black,fill=btdm@background},V/.style={-Kite,lightgray,line cap=round,line width=3.5pt}]
   \node[W] (a) at(0,0) {7};
