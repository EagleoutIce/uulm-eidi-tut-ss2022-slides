...
\end{plainjava}
\begin{tikzpicture}[overlay,remember picture]
    \node[above left,yshift=\btdmfootheight,text width=14cm,scale=.8] at(current page.south east) {\begin{alignat*}{4}
        \ms{a\text{++},~~a\text{-\,-}}&\to~\ms{!a,~~ -a,~~ \text{++}a,~~\text{-\,-}a}&&\to~\ms{a~*~b,~~a~/~b,~~a~\%~b} \\
        & \to~\ms{a+b,~~a-b}&&\to~\ms{a==b,~~a<b,~~\ldots}\\
        & \to~\ms{a~\mbox{\textasciicircum}~b}&&\to~\ms{a~\&\&~b}\\
        & \to~\ms{a~||~b}
     \end{alignat*}};
\end{tikzpicture}
\end{frame}
