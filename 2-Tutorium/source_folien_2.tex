...
\title{Diese Worte \texorpdfstring{\,\setbox0=\hbox{getau\kern5pt\relax scht}\phantom{\copy0}\llap{\tikz[baseline=(k.base)]{%
\packtitlenode0k
...
\iffull{\SummaryFrame
\def\sub#1#2{\node[font=\footnotesize\sffamily,scale=.715,align=center,gray,below=-3.65mm] at(#1.south) {\strut#2\strut};}
...
\SetNextSectionText{Es gibt keine if-Schleifen, sondern nur if-Abfragen!\\--- \sectiontextlink\link{http://if-schleife.de/}{if-schleife.de}}
...
\end{itemize}
\begin{tikzpicture}[@O]
    \onslide<13->{\node[right,T] at(semi.east) {Das \T{\textbf{;}}~nicht vergessen!};}
...
\begin{enumerate}
    \item<2-> \task{Entwerfen Sie einen Algorithmus, der gegeben einer Liste und zwei Indizes die entsprechenden Elemente
...
\def\K#1{\textbf{\paletteC{\small\itshape\only<20->{\color{codeouthl}}#1}}}% update anim slide
...
\begin{frame}{Aufgabe 2: Datentypen}
    \task<2->{Welchen Datentyp würden Sie wählen um folgende Daten zu speichern? \only<3->{\expandafter\textbf}{Begründen Sie Ihre Antwort!}}\smallskip
    \begin{enumerate}
        \itemsep12pt
...
\end{tikzpicture}
\end{frame}
\def\rr#1#2#3#4{[\(\overset{{\text{\scriptsize$\color{codeouthl}#3$}}}{\num{#1}}\) bis \(\overset{{\text{\scriptsize$\color{codeouthl}#4$}}}{\num{#2}}\)]}
\def\r#1--#2[#3]{\rr{-#1}{#2}{-2^{#3}}{2^{#3}-1}}%
\def\appr#1#2{\num{#1} Sekunden sind \(\approx\num{#2}\) Stunden}
\def\apprJ#1#2{\num{#1} Sekunden sind \(\approx\num{#2}\) Jahre}
\begin{frame}{Primitives Fassungsvermögen\hfill Ganzzahlwerte}
\begin{itemize}[<+(1)->]
    % \item Wir vermerken (die Grenzen sind inklusiv)
    \itemsep4pt
    \item \bjava{byte}\pause\ fasst \r128--127[7]\pause\infoblock{Das reicht aus, um alle Primzahlen zwischen~\num{2000} und~\num{3000} zu zählen. \appr{127}{0.035}.}
    \item \bjava{short}\pause\ fasst \r32768--32767[15]\pause\infoblock{\appr{32767}{9.1}.}
    \item \bjava{char}\pause\ fasst \rr{0}{65535}{0}{2^{16}-1}\pause\infoblock{Das reicht für alle Code Units von UTF-16!}
    \item \bjava{int}\pause\ fasst \r2147483648--2147483647[31]\pause\infoblock{\apprJ{2147483647}{68.05}.}
    \item \bjava{long}\pause\ fasst \r9223372036854775808--9223372036854775807[63]\pause\infoblock{\apprJ{9223372036854775807}{292271023045.3} \(\approx 658\) Millionen mal \say{älter}, als das Universum.}
\end{itemize}
\begin{tikzpicture}[@O]
    \onslide<17->{\node[above left,yshift=\btdmfootheight,codeouthl] at(current page.south east) {\footnotesize\link{https://docs.oracle.com/javase/specs/jls/se17/html/jls-4.html\#jls-4.2.1}{JLS17~$4.2.1$}, \link{https://docs.oracle.com/javase/specs/jvms/se17/html/jvms-2.html\#jvms-2.3.1}{JVMS17 2.3.1}};}
\end{tikzpicture}
\end{frame}
\begin{frame}{Primitives Fassungsvermögen\hfill Fließkommawerte}
\begin{itemize}[<+(1)->]
    % \item Wir vermerken (die Grenzen sind inklusiv)
    \itemsep10pt
    \item \bjava{float}\pause\ schweift zwischen \(\pm 2^{-126}\) bis zu \((2 - 2^{-23}) \cdot 2^{127}\)\pause\infoblock{Joa\ldots Das sind mehr als \(5 \cdot 10^{30}\) Jahre.}
    \item \bjava{double}\pause\ schweift zwischen \(\pm 2^{-\num{1022}}\) bis zu \((2 - 2^{-52}) \cdot 2^{\num{1023}}\)\pause\infoblock{So zählen sie sich leicht, die Atome des Universums!}\bigskip
    \item Dabei sollten wir die Rundungsprobleme aber nicht außer Acht lassen
    \item Jenseits des Primitiven liefert Java natürlich mehr \begin{itemize}
        \item \solGet{keywordB}{\link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/math/BigInteger.html}{BigInteger}} begrenzt die Anzahl Bits durch einen \bjava{int}
        \item \solGet{keywordB}{\link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/math/BigDecimal.html}{BigDecimal}} erlaubt Skalieren durch \bjava{int} und volle Kontrolle übers Runden
    \end{itemize}
\end{itemize}
\end{frame}
}
...
\begin{tikzpicture}[@O]
    \onslide<8->{\node[above left,yshift=\btdmfootheight,text width=15.5cm,scale=.6] at(current page.south east) {\begin{alignat*}{5}
        ...
        & \to~\ms{\textcolor{gray}{a~||~b}} &&\to \ms{\ldots}
     \end{alignat*}};}
    \color{lightgray}
...
\begin{itemize}
    ...
    \item<3-> In Aufgabe~1 suchen wir \textit{einen} booleschen Ausdruck der das Programm abbildet. \onslide<4->{\infoblock{Vergleiche hierzu die Präsenzaufgabe von letzter Woche.}}
...
\begin{frame}{Aufgabe 4: Iterative Wurzelberechnung}
    \begin{itemize}[<+(1)->]
        ...
        \item Ihr dürft gerne eure Entscheidungen in den Kommentaren begründen.
        \item An welcher Stelle kommen Unteralgorithmen zum Einsatz?\infoblock{Und\ldots Wo könnten sie noch hilfreich sein?}
    \end{itemize}
\end{frame}
...
\SetNextSectionText[.55\linewidth]{Der zeitliche Abstand [\ldots] läßt den wahren Sinn, der in einer Sache liegt, erst voll herauskommen. Die Ausschöpfung des wahren Sinnes aber, der in einem Text oder einer künstlerischen Schöpfung gelegen ist, kommt nicht irgendwo zum Abschluß, sondern ist in Wahrheit ein unendlicher Prozeß.\\--- Hans-Georg Gadamer~\cite[unverändert, p.~282]{hansWahrheit}}
...
\begin{itemize}[<+(1)->]
    ...
    \item \bjava{Integer.parseInt("13")} liefert die vom \bjava{String} beschriebene Ganzzahl (\snum{13})
    ...
    \item Javas implizite Typkonvertierungen existieren nur für primitive Datentypen \begin{itemize}
        \item \bjava{byte}~\(\to\)~\bjava{short}~\(\to\)~\bjava{int}~\(\to\)~\bjava{long}~\(\to\)~\bjava{double}~~und~~\bjava{char}~\(\to\)~\bjava{int}
...
\outro{\vskip6mm\centering\begin{tikzpicture}[scale=1.85]
    \only<2->{\pingu[wings=shock,body type=chubby,eyes hearts=pingu@red!10!pingu@black, santa hat=btdm@primary, santa beard]}
\end{tikzpicture}}
