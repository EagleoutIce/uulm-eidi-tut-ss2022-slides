...
\end{algorithm}}}
\begin{tikzpicture}[overlay,remember picture,T/.style={sol@colors@lst@comments,font=\itshape\sffamily\footnotesize}]
    \onslide<16->{\node[above left,T,align=left,text width=6cm,yshift=\btdmfootheight+1.85cm] at(current page.south east) {Angenommen \(N\) sei \(5\), dann gilt für die Anzahl innerer Schleifendurchläufe:\smallskip\\
    \columns[onlytextwidth,c]
    ...
    \onslide<21->{\node[above left,yshift=\btdmfootheight,text width=.6\linewidth] at(current page.south east) {\sbseries\footnotesize\begin{align*}
        ...
        \color{btdm@text}\only<22->{={}& \color{btdm@text}-1\E + 3\cdot N\E + 5\cdot N^2\E - 5\cdot N\E}\strut \\[-.25mm]
...
\begin{frame}{Aufgabe 2: Datentypen}
    \task<2->{Welchen Datentyp würden Sie wählen um folgende Daten zu speichern? \only<3->{\textbf}{Begründen Sie Ihre Antwort!}}
    \begin{enumerate}
        \itemsep8pt
        \item<4-> \task{Die Email-Adresse einer Person}
        \item<5-> \task{Den Akkustand eines Smartphones}
        \item<6-> \task{Den Notendurchschnitt einer Prüfung}
        \item<7-> \task{Ein Satzzeichen}
    \end{enumerate}
\end{frame}}
