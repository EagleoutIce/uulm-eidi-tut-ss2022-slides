...
\def\to{~\raisebox{1pt}{\;\tikz\draw[lightgray,-{Kite[scale=1.05]},line cap=round,line width=.925pt] (0,0) -- (6mm,0);}~}%
...
\begin{plainjava}[lineskip=15pt]
!*\onslide<3->*!boolean b = true;
!*\onslide<4->*!b = !(true || false) || b;
!*\onslide<5->*!b = !(true && false);
!*\onslide<6->*!b = (!(true && true) || (false || false));
!*\onslide<7->*!b = !b;
\end{plainjava}
\begin{tikzpicture}[overlay,remember picture]
    \node[above left,yshift=\btdmfootheight,text width=15.5cm,scale=.65] at(current page.south east) {\begin{alignat*}{5}
        \ms{a\text{++},~~a\text{-\,-}}&\to~\ms{!a,~~ -a,~~ \text{++}a,~~\text{-\,-}a}&&\to~\ms{a~*~b,~~a~/~b,~~a~\%~b} && \to~\ms{a+b,~~a-b} \\
        &\to~\ms{a==b,~~a<b,~~\ldots} && \to~\ms{a~\mbox{\textasciicircum}~b}&&\to~\ms{a~\&\&~b}\\
        & \to~\ms{a~||~b}
     \end{alignat*}};
\end{tikzpicture}
\end{frame}
