...
\only<2-|handout:1>{6.~Gesicherte Fehlermeldungen\only<handout:0>{\smallskip\\}}
...
\end{onlyenv}
\begin{itemize}
   \item<14-|handout:2> Die Klassen finden sich dann hier: \only<14->{\textattachfile{\curpath BinaerUmwandler.java}{BinaerUmwandler.java}} und \only<14->{\textattachfile{\curpath BinaerHaupt.java}{BinaerHaupt.java}}.
\end{itemize}
\end{frame}
\SidebarReset
...
{
\SidebarSolution
\newcommand<>\ComplexityMarker[1]{\only#2{\rlap{\quad\compl{#1}}}}
...
\end{align*}
\onslide<11->{Das heißt, konkrete Implementationen können durch Anpassungen und Unteralgorithmen unterschiedliche Laufzeitkomplexitäten berechnet werden.}
\end{frame}
\SidebarReset
}
