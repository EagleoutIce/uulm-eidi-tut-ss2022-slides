...
\UsePalette{Recap}
\fancyulcolor{paletteA!62!pingu@black!36!pingu@white}
...
\begin{frame}{What can we say about an Algorithm?}
   \vfill
   \onslide<2->{\centerline{\Large\bfseries\color{gray}\sbseries Totale Korrektheit}}
   \vfill
...
\end{equation*}
\onslide<3->{Dabei ist \(b_i\) der \(i\)-te Eintrag des Strings. Für \(b = \text{\T{1001}}\) gilt damit: \begin{equation*}
...
\begin{tikzpicture}[@O]
   % macro? pffffh
   \onslide<3->{\draw[decoration=brace,decorate] ([yshift=-1mm]@1.south east) to[edge node={node[below=1mm] {8}}] ([yshift=-1mm]@1.south west);
...
\begin{itemize}
   \itemsep10pt
   \item<3-> \bjava{14 - 3D * 2 / 12}
   \item<3-> \bjava{(char) ((byte) 'a' + 3) + " Sonne"}
   \item<3-> \bjava{14 - 2 + (3 > 9 || true ^ false ? "Hallo" : "Welt")}
   \item<3-> \bjava{7/2 - 3 * (float) 2.5 + 2f}
\end{itemize}}
\onslide<1->{\ShortSide}%
\end{frame}
\SidebarReset
...
\begin{uncoverenv}<2->
\flushleft Im Folgenden finden Sie zwei Code-Ausschnitte. Schreiben Sie diese so um, dass sie nur noch Schleifen der angegebenen Art verwenden, ohne dass sich das Verhalten des Codes verändert. \onslide<3->{Dabei repräsentiert \bjava{k} eine natürliche Zahl (exklusive \(0\)), welche Sie nicht kennen.}
\lstfs{8}\begin{columns}[onlytextwidth,T]
\column{.65\linewidth}
\begin{plainjava}
!*\onslide<4->*!// In eine do-while-Schleife
...
\column{.35\linewidth}
\begin{plainjava}
!*\onslide<5->*!// In eine for-Schleife
!*\onslide<5->*!int i = 1;
!*\onslide<5->*!do {
!*\onslide<5->*!    i *= k;
!*\onslide<5->*!    k -= i;
!*\onslide<5->*!} while(k >= 5);
!*\onslide<5->*!System.out.println(i);
\end{plainjava}
\end{columns}
\end{uncoverenv}
\onslide<1->{\ShortSide}%
...
\only<2-12|handout:1> {1.~Klasse \T{BinaerUmwandler} mit privaten Instanzvariablen\smallskip\\
\onslide<5-12|handout:1> {2.~Getter und Setter}}
\only<13-|handout:2> {3.~\bjava{BinaerUmwandler()} und \bjava{BinaerUmwandler(String)}}
\medskip
\SetupLstHl
...
\end{minted}
\end{onlyenv}\onslide<1->
\end{frame}
\SidebarReset

\SidebarTask{Die Algorithmenklasse, 2}
\begin{frame}[c]{Die Algorithmenklasse\rhead{II}}
\onslide<2->{%
\updateitemize{1.}\begin{enumerate}[<+(1)->]
   \setcounter{enumi}{3}
   \item Implementieren Sie Ihren Algorithms aus \TaskRef{Ein Algorithmenentwurf} als die statische Funktion \bjava{long binZuDez(String binary)}:\medskip\par
   \begin{algorithm}[H]
\PreCode
    \KwIn{binary string \(b\)}
\StartCode
    \(N \gets \text{length of }b\)\;
    \(d \gets 0\)\;
    \For{\(i \gets 1\) \KwTo~\KwSty{inclusive} \(N\) \tikzmarknode{@-main}{\KwStep} \(1\)} {
    ...
    \KwRet \(d\)\;
\end{algorithm}
   \item Ändern Sie den Binärstring-Setter und den Konstruktor so ab, dass er die Dezimalzahl entsprechend aktualisiert.
\end{enumerate}}
\onslide<1->{\ShortSide}%
\end{frame}
\SidebarReset
...
\only<2-|handout:1>{4.~Algorithmus implementieren\only<handout:0>{\smallskip\\}}
\only<12-|handout:2>{5.~Anpassen von Setter \& Konstruktor}
\medskip
\SetupLstHl
\begin{tikzpicture}[@O]
   \node[below left,text width=.7\linewidth,scale=.65] at (current page.north east) {\begin{algorithm}[H]
\PreCode
    \KwIn{binary string \(b\)}
\StartCode
    \(N \gets \text{length of }b\)\;
...
\end{onlyenv}\onslide<1->
\begin{tikzpicture}[@O]
   \onslide<0|handout:1>{\node[above left=.5cm,yshift=.9cm,T,align=right] at(current page.south east) {Alternativ auch \T{int bi = binary.charAT(i) - '0';}.\\Die \T{i - 1} brauchen wir, da im Pseudocode \T{i = 1} gilt,\\Java aber mit 0 indiziert.};}
\end{tikzpicture}
\end{frame}
\SidebarReset
...
\onslide<2->{\footnotesize\flushleft Sie sehen das Grundgerüst einer Kameraklasse. Sie dürfen für die Aufgabe \textit{keine} (Java-)Ausnahmen verwenden.\updateitemize{1.}
\begin{enumerate}
    \item<3-> Schreiben Sie einen leeren Konstruktor, welcher die von Java zugewiesenen Standardwerte explizit initialisiert. Dieser Konstruktor soll nicht von Code abseits der \bjava{Camera}-Klasse aufrufbar sein und so verhindern, dass weitere Kamera-Objekte erstellt werden können.
    \item<4-> Erweitern Sie die Klasse nun in sofern, dass es immer maximal fünf Kamera-Objekte gibt, welche durch \bjava{get(int)} durch eine Zahl \(x \in \{0, 1, 2, 3, 4\}\) abgefragt werden können. Dabei soll ein Kamera-Objekt erst (mit Ihrem vorher erzeugten Konstruktor) erzeugt werden, wenn es durch die Zahl angefragt wird. Weiter sollen Folgeanfragen mit der Zahl dasselbe Objekt zurückliefern. Geben Sie für \(x < 0\) und \(x \geq 5\) den Wert \bjava{null} zurück.
\end{enumerate}}
\onslide<1->{\ShortSide}%
\end{frame}
...
\SidebarTask{Die Algorithmenklasse, 3}
\begin{frame}[c]{Die Algorithmenklasse\rhead{III}}
\onslide<2->{Nehmen Sie ihren Code aus \TaskRef{Die Algorithmenklasse, 2} als Basis.\smallskip%
\updateitemize{1.}\begin{enumerate}[<+(1)->]
   \setcounter{enumi}{5}
   \itemsep5pt
   \item Sichern Sie ihre Klasse so ab, dass \bjava{NullPointerException}s \say{sinnvoll} abgefangen werden.
   \item Legen Sie eine Klasse \T{BinaerHaupt} an und schreiben sie eine Programmeinstiegsmethode, welche ein \T{BinaerUmwandler}-Objekt erzeugt und die errechnete Dezimalzahl ausgibt.
\end{enumerate}}
\onslide<1->{\ShortSide}%
\end{frame}
\SidebarReset
...
\only<2-7|handout:1>{6.~Gesicherte Fehlermeldungen}
\only<8-|handout:2>{7.~Die Hauptklasse \T{BinaerHaupt}}
\SetupLstHl
...
\end{onlyenv}
\begin{itemize}
   \item<14-|handout:2> Die Klassen finden sich dann hier: \only<14-|handout:2>{\textattachfile{\curpath BinaerUmwandler.java}{BinaerUmwandler.java}} und \only<14-|handout:2>{\textattachfile{\curpath BinaerHaupt.java}{BinaerHaupt.java}}.
\end{itemize}
\end{frame}
\SidebarReset
...
Identifizieren Sie \textsb{acht} dieser Fehler durch \smallskip\updateitemize{1.}\begin{enumerate}
    \item<3-> die Angabe der Code-Zeilennummer und
    \item<4-> eine kurze Erklärung, was den Fehler verursacht.
\end{enumerate}
\flushleft
\onslide<5->{Wenn Sie mehrere Fehler in einer Code-Zeile finden, tragen sie diese separat ein.
...
\SidebarTask{Noch komplexer!}
\begin{frame}[fragile,c]{Noch komplexer!}
\onslide<2->{Geben Sie die worst-case Laufzeitkomplexität (abhängig von der Eingabelänge) des folgenden Algorithmus an:}
\begin{minted}[escapeinside={/*}{*/}]{java}
/*\onslide<3->*/public static long binZuDez(String binary) {
/*\onslide<4->*/   int n = binary.length();
/*\onslide<4->*/   long d = 0;
/*\onslide<4->*/   for(int i = 1; i <= n; i++) {
/*\onslide<4->*/      int bi = binary.charAt(i - 1) - '0';
/*\onslide<4->*/      d = d + bi * Math.pow(2, n - i);
/*\onslide<4->*/   }
/*\onslide<4->*/   return d;
/*\onslide<3->*/}
\end{minted}
\onslide<5->{In wie weit unterscheidet sich ihre Berechnung von der aus \TaskRef{Eine Aufwandsanalyse}?}
   \onslide<1->{\ShortSide}%
\end{frame}
\SidebarReset
...
{
\SidebarSolution
\newcommand<>\ComplexityMarker[1]{\only#2{\rlap{\quad\smash{\compl{#1}}}}}
\def\compl#1{\text{\bfseries\itshape\paletteA{#1}}}
\def\<{\ensuremath{\langle}}
\def\>{\ensuremath{\rangle}}
\begin{frame}[fragile,c]{Das ist ja ganz was anderes!}
\begin{minted}[escapeinside={/*}{*/}]{java}
/*\onslide<2->*/public static long binZuDez(String binary) {
/*\onslide<2->*/   int n = binary.length(); /*\ComplexityMarker<3->{2}*/
/*\onslide<2->*/   long d = 0; /*\ComplexityMarker<4->{1}*/
/*\onslide<2->*/   for(int i = 1; i <= n; i++) { /*\ComplexityMarker<5->{\paletteD{n}}*/
/*\onslide<2->*/      int bi = binary.charAt(i - 1) - '0';  /*\ComplexityMarker<6->{$\langle$charAt$\rangle$ + 3}*/
/*\onslide<2->*/      d = d + bi * Math.pow(2, n - i); /*\ComplexityMarker<7->{$\langle$pow$\rangle$ + 4}*/
/*\onslide<2->*/   }
/*\onslide<2->*/   return d; /*\ComplexityMarker<8->{1}*/
/*\onslide<2->*/}
\end{minted}
\smallskip
\begin{align*}
   &\onslide<9->{\compl{2}\,E + \compl{1}\,E + \compl{\paletteD{n}} \cdot (\compl{\<\text{\T{charAT}}\> + 3}\,E + \compl{\<\text{\T{pow}}\> + 4}) + \compl{1}\,E}\\
   \onslide<10->{=}& \onslide<10->{\bigl(4 + (7 + \<\text{\T{charAT}}\> + \<\text{\T{pow}}\>) \cdot n\bigr)\,E}
\end{align*}
\onslide<11->{\scriptsize Das heißt, für konkrete Implementationen können durch Anpassungen und Unteralgorithmen unterschiedliche Laufzeitkomplexitäten berechnet werden.\par}
\end{frame}
\SidebarReset
}

\iffull
\outro{\rlap{Basiert auf der \def\fancyulbackground{paletteA}\fancyulcolor{pingu@white}\link{https://github.com/EagleoutIce/christmas-eidi-recap}{Weihnachtswiederholung}}}
\else
