...
\onslide<2->{\footnotesize\flushleft Sie sehen das Grundgerüst einer Kameraklasse. Sie dürfen für die Aufgabe keine (Java-)Ausnahmen verwenden.
\begin{enumerate}
...
\updateitemize{1.}%
\begin{enumerate}
   \item<4-> Leerer Konstruktor, der Standardwerte explizit zuweist und nicht von außerhalb aufrufbar sein soll.
\end{enumerate}
\endcolumns
\updateitemize{1.}%
\begin{enumerate}
   \setcounter{enumi}{1}
   \item<5-> Maximal fünf Kamera-Objekte (\(\{0, \ldots, 4\}\)) durch \bjava{get(int)}. Sollen erst erzeugt werden, wenn angefordert, illegale Werte sollen \bjava{null} liefern.
\end{enumerate}
\onslide<1->{\ShortSide}%
\end{frame}
\SidebarReset
...
\begin{frame}[c]{Klassische Fehler}
   \begin{itemize}
      \itemsep10pt
      \item<2-> \link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/NullPointerException.html}{\solGet{keywordC}{NullPointerException}}\onslide<3->{\t{unchecked}
      \infoblock{Wir verwenden \T{null} wo ein Objekt gebraucht wird.}}
      \item<4-> \link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/ArithmeticException.html}{\solGet{keywordC}{ArithmeticException}}\onslide<5->{\t{unchecked}
      \infoblock{Verbotene Rechenoperation wie Teilen einer Ganzzahl durch \(0\).}}
      \item<6-> \link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/io/FileNotFoundException.html}{\solGet{keywordC}{FileNotFoundException}}\onslide<7->{\t{checked}\infoblock{Der gegebene Pfad liefert keine Datei!}}
      \item \link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/StackOverflowError.html}{\solGet{keywordC}{StackOverflowError}}
      \item \solGet{keywordC}{IllegalArgumentException}
      \item \solGet{keywordC}{ArrayIndexOutOfBoundsException}
   \end{itemize}
\end{frame}
}
...
% \begin{layout-full}
% \end{layout-full}
\lstset{lineskip=0.5pt}\soldisablenumhl
\begin{plainvoid}[numbers=left,numbersep=6pt,multicols=2,numberstyle=\scriptsize\color{black}]
...
\begin{frame}[c]{Fehlersuche\rhead{II}}
   ...
   \begin{enumerate}[<+(1)->]
      ...
      \item[9.] \textit{Zeile 30}:\quad \bjava{global} ist in Java kein gültiges Methoden-\\\qquad Präfix.
...
\begin{frame}
   ...
   TODO: exceptions (Beispiele vorstellen, Baum machen)
\end{frame}
