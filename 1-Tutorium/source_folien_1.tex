...
\begin{itemize}[<+(1)->]
    ...
    \item Wir haben die Geburt von Variablen sowie ihre Wachstumsphase kennengelernt: \begin{itemize}
        ...
        \item Die \textit{Initialisierung} \bjava{int x; x = 5} beschreibt die erste Wertzuweisung (sie macht die Variable verwendbar)
        \item Eine \textit{(Wert-)Zuweisung} \bjava{x = 3} ist jede weitere Änderung des gespeicherten Wertes.
    \end{itemize}
    \item In die Wahl des richtigen Datentyps fließen viele Informationen mit ein.
\end{itemize}
\end{frame}
}
