...
\vspace*{-.5\baselineskip}\begin{enumerate}[(1)]
    \item\only<2>{\label{algB:a}} Setze $\DeltaMax$ auf $-\infty$.
    \item\only<2>{\label{algB:b}} Setze $i$ auf $1$.
    \item\only<2>{\label{algB:c}} Solange $(i < n)$, wiederhole:\smallskip\\
            \quad Setze \(test\) auf \(\sfrac{(p_{i + 1} - p_i)}{p_i}\).\smallskip\\
            \quad Wenn \((test > \DeltaMax)\):~~Setze \(\DeltaMax\) auf \(test\).\smallskip\\
            \quad Erhöhe \(i\) um \(1\).
...
\begin{itemize}
    ...
    \item<9->[\paletteA{\mdseries\faAngleRight}] Damit erhalten wir: \(2 + (n - 1) \cdot ( 3+ 2 + 1) + 2 + 2 = 6n - 2\) (also \(\O(n)\)).
\end{itemize}
