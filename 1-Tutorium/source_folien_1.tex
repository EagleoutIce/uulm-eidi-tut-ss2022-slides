...
\null\hfill\smash{\raisebox{-.5cm}{\scalebox{0.8}{\begin{tikzpicture}% [overlay,remember picture]
...
\onslide<2->{Wir zeigen nur die partielle Korrektheit \info{\say{Verifikation}}.}\medskip\par
\small\onslide<3->{Für ein beliebiges aber festes \(n \in \N\) mit \(n \geq 2\) und der Eingabe \(p_1, \ldots, p_n\)} \onslide<4->{zeigen wir per vollständiger Induktion über \(i\),} \onslide<5->{dass in jedem Schritte gelte:\smallskip\par \centerline{\(\displaystyle\DeltaMax \geq M(i) \qquad M(i) = \max_{1 \leq j \leq i}\left\{\frac{(p_{j + 1} - p_j)}{p_j}\right\}\)}}
\endcolumns\medskip\par
\small
    \begin{description}[xIA:]% Inudktionshypothese als Voraussetzung und Behauptung
        \itemsep0pt
        \item<6->[IA:] Mit \(i = 1\) ist \(\DeltaMax = (p_2 - p_1)/p_1\) das triviale Maximum.
        \item<7->[IH:] Die Aussage \(\DeltaMax \geq M(i)\) gilt für ein \(i\).
        \item<8->[IS:] Es gilt \(\DeltaMax \geq M(i)\) für \(i\) (\textit{IH}). \onslide<9->{Wir zeigen, dass auch \(\DeltaMax \geq M(i + 1)\) gilt:}
        \begingroup\footnotesize
        \begin{enumerate}
            \item<9-> \(test > \DeltaMax\): Durch die Bedingung wird \(\DeltaMax\) aktualisiert. Es gilt: \(\DeltaMax = test \geq M(i + 1)\).
            \item<10-> \(test \leq \DeltaMax\): Der Anstieg durch \(i + 1\) ist kein neues Maximum. Es gilt: \(\DeltaMax \geq M(i + 1) \geq test\).
        \end{enumerate}\endgroup
    \end{description}
    \onslide<12->{Nach \(n-1\) Schritten \info{durch \(i < n\)}, werden alle \(p_1, \ldots, p_n\) betrachtet} \onslide<13->{~\(\implies\DeltaMax\) ist Maximum aller.}
\end{frame}}
\fi
...
{
\AddonFrame
\def\incr{\(+\llap{\smash{\raisebox{1.5pt}{$+$}}\kern-3pt}\kern2pt\)}
...
\begin{frame}[c]{Tabellarische Aufwandsanalyse}
    ...
    \scriptsize\onslide<3->{\begin{tabular}{lccl}
        ...
        \onslide<9->{Ergebnis ist $100 \cdot \DeltaMax$} & \onslide<9->{2\,E} & \onslide<9->{\(1\)} & \onslide<9->{\(\cdot, \color{gray}\rightarrow\)}  \\
        \bottomrule
    \end{tabular}}
