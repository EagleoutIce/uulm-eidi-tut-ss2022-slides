...
\begin{frame}{Laufzeitabschätzungen}
    \begin{itemize}[<+(1)->]
        ...
        \begin{itemize}
            ...
            \item Eine pessimistische Abschätzung liefert das \say{Amdahlsche Gesetz}~\info{wer mag:~\cite{amdahl1967amdahl}}.
        \end{itemize}
...
\begin{frame}[c]{Aufgabe 3}
    ...
    Sie stellen jedoch fest, dass die \g{Preise für elektronische Geräte} durch den \g{Chipmangel} in \g{letzter Zeit} stark gestiegen sind.
    Nun haben Sie eine \g{Website} gefunden, die Ihnen die jeweiligen \g{\k{Tagespreise}} eines Tablets über die \g{letzten Monate} liefert.
    Sie möchten nun den \g{\k{maximalen pro\rlap{-}}} \g{\k{zentualen Preisanstieg}} zwischen \g{\k{zwei aufeinanderfolgenden Tagen}} für dieses Tablet herausfinden.\bigskip\par
...
\begin{description}[Tag]
    ...
    \item<+(1)->[Zwei aufeinanderfolgende Tage:] zwei aufeinanderfolgende Listenindizes: \(i\) und \(i + 1\) wobei \(i, i + 1 < n\).
    \item<+(1)->[Preisanstieg:] positive Veränderung einer reellen Zahl (dem Tagespreis): \(p_{i + 1} - p_i > 0\).
    \item<+(1)->[maximaler prozentualer Preisanstieg:] größter relativer Preisanstieg: \(\max_{1 \leq i < n} \frac{p_{i + 1} - p_i}{p_i} \cdot 100\).
\end{description}
\infoblock{\textbf{Wichtig:} Wir sind hier nicht in einer Programmiersprache: Typen wie \say{\bjava{int}} gibt es nicht $\implies$ mathematische Notation benutzen oder eigenen Typ definieren (wie Tupel, \ldots)!}
\end{frame}}
...
\outro{\vskip6mm\centering\begin{tikzpicture}[scale=2.5]
    \only<2->{\pingu[monocle left,laptop right,cane left,tie,body type=legacy,headphones=pingu@green!80!pingu@black]}
\end{tikzpicture}}
