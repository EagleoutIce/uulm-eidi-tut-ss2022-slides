...
\begin{frame}{Allgemeines}
    \begin{itemize}[<+(1)->]
        ...
        \begin{itemize}[<+(1)->]
            \item Vorteile einer Kopie (gegenüber in-place): Nebeneffekte von Methoden sind generell schlecht! \say{out-Parameter} sollten vermieden werden (wo nicht unbedingt notwendig).
            \item Nachteile einer Kopie (gegenüber in-place): Der benötigte Speicher wird verdoppelt (was, wenn das Array riesig ist?)
        \end{itemize}
...
\begin{frame}[t,fragile]{Kopie-Varianten}
    \begin{itemize}[<+(1)->]
        \itemsep10pt
        \item Deswegen unterscheiden wir zwei Arten von Kopien: \emph{shallow} und \emph{deep}.
            \infoblock{Genau genommen gibt es noch viel mehr, wie zum Beispiel \emph{lazy}, aber das soll uns hier nicht weiter stören.}
...
\end{plainjava}
        ...
        \item Eine \emph{deep copy} würde rekursiv auch neue Objekte von \bjava{Y} und \bjava{Z} erzeugen.\pause{} \infoblock{Quizfrage: was passiert oder besser, was kann alles passieren, wenn \bjava{Y} wieder eine Referenz auf \bjava{X} enthält?}
...
\begin{frame}[t,fragile]{Cloneable}
    \begin{itemize}[<+(1)->]
        ...
        \item Aus dem \link{https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/Cloneable.html}{Java-Vertrag}:\par\pause
        \textit{[\ldots]~Note that this interface does not contain the clone method.
        Therefore, it is not possible to clone an object merely by virtue of the
        fact that it implements this interface. \onslide<+(1)->{Even if the clone method is invoked \link{https://www.oracle.com/technical-resources/articles/java/javareflection.html}{reflectively}, there is no guarantee that it will succeed. [\ldots]}}
    \end{itemize}
\end{frame}
...
\begin{frame}[t,fragile]{Long story short}
    \begin{itemize}[<+(1)->]
        ...
        \item Bei Arrays sind die Kopien mit \bjava{clone} immer shallow.
