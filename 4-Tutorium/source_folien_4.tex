...
\vspace*{-.15\topsep}\begin{columns}[onlytextwidth,c]
...
\end{itemize}
\begin{tikzpicture}[@O]
\onslide<8->{\node[T,right=-1pt] at(@1.east) {\faCheck~\T{m1} ist noch nicht initialisiert};}
\onslide<10->{\node[T,right=-1pt,align=left] at(@2.east) {\faFlash~Java \textup{muss} das Array initialisieren~\(\to\) braucht dessen Größe};}
\onslide<12->{\node[T,right=-1pt] at(@3.east) {\faCheck~\T{m3} ist \T{\{ null, null \}} (Default-Wert für \T{double[]})};}
\onslide<14->{\node[T,right=-1pt] at(@4.east) {\faCheck~\T{m4} ist \T{\{\;\{ 0, 0, 0 \}\;\}} (Default-Wert für \T{int})};}
\onslide<16->{\node[T,below right,xshift=-1pt,yshift=.6\baselineskip,align=left] at(@5.east) {\faFlash~\T{null} und \T{\{\}} \info{leer} sind valide \T{double[]},\\\phantom{\faFlash}~\T{\{ null \}} aber nicht, da \T{double} nicht \T{null}\\\phantom{\faFlash}~sein kann.};}
\end{tikzpicture}
\end{frame}
...
public static double berechneZeilensummennorm(double[][] mat) {
    ...
    for (double[] array : mat) {
        ...
        if (norm == -1.0):ws::ws::ws::ws::ws::ws::ws::ws:return -1.0; // Fehler in Unteralgorithmus
...
\end{plainjava}
    \item<4-> Der gesamte Code findet sich in \only<4->{\textattachfile{\curpath Matrix.java}{Matrix.java}}
...
\begin{frame}{Aufgabe 3: Numerische Integration}
    TODO: berechne TtrapezFlaeche prüfe auf < 0 und so optional, je nachdem wie man es betrachtet (eigenständig, nicht....)
