...
\column{.35\linewidth}
\begin{minted}[escapeinside={/*}{*/}]{java}
/*\CodeMarker{\textbf{Head}-Recursive}*/
...
\column{.45\linewidth}
\begin{minted}[escapeinside={/*}{*/}]{java}
/*\CodeMarker{\textbf{Tail}-Recursive, call as f(x, 1)}*/
...
\end{columns}\vspace*{-\smallskipamount}
        \item Ruft sie sich auch mehrfach pro Rekursionsfall auf, ist sie \textit{verzweigt rekursiv}.
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{tabular}{ll*2{l}>{\footnotesize}{l}}
    ...
    \onslide<3->{\(\checkmark\)} & \onslide<3->{\tikzmarknode{itstart}{Bubble}}      & \onslide<3->{\(\O(n^2)\)\textcolor{gray}{, \(\O(n)\)}}       & \onslide<3->{\(\O(n^2)\)} & \onslide<4->{Vertausche benachbarte El., solange unsortiert.} \\
    \onslide<5->{\(\checkmark\)} & \onslide<5->{Insertion} & \onslide<5->{\(\O(n)\)}             & \onslide<5->{\(\O(n^2)\)}      & \onslide<6->{Sortiere 1. unsortiertes El. in sortierten Teil ein.}\\
    ...
    \onslide<9->{\(\checkmark\)} & \onslide<9->{\tikzmarknode{rekstart}{Merge}}    & \onslide<9->{\(\O(n \log n)\)} & \onslide<9->{\(\O(n \log n)\)}  & \onslide<10->{Aufteilen bis einel., wiederholtes mergen sortierter Teillisten.}\\
...
\begin{description}[Einfach verketteter Binärbaum:]
    \itemsep25pt
    \item<3->[Einfach verkettete Liste:] ~\onslide<4->{\tikzset{W/.style={draw, rounded corners=2pt},K/.style={W, rectangle split, rectangle split parts=2,rectangle split horizontal}}\scalebox{.8}{\begin{tikzpicture}[align-half-base]
    \node[K] (a) at (0,0) {4\nodepart{two}};
...
% this is horrible, but i have no time
    \item<5->[Doppelt verkettete Liste:] ~\onslide<6->{\tikzset{W/.style={draw, rounded corners=2pt},K/.style={W, rectangle split, rectangle split parts=3,rectangle split horizontal}}\scalebox{.8}{\begin{tikzpicture}[align-half-base]
    \node[K] (a) at (0,0) {\nodepart{two}4\nodepart{three}};
...
\end{tikzpicture}}}
    \item<7->[Einfach verketteter Binärbaum:] \tikzset{W/.style={draw, rounded corners=2pt}}%
    \onslide<8->{\smash{\raisebox{-.5\height}{\rotatebox{90}{\scalebox{.6}{\begin{tikzpicture}[align-half-base]
