...
\AnimateCode{onslide={2, 3}, first slide=15}
\begin{minted}[morekeywords={[3]{Stack}}]{java}
/*\CodeFileMarkerAttach<13->{Stack.java}*/
...
\end{minted}
\endAnimateCode
\AnimateCode{onslide={1, 3}, first slide=15}
\begin{tikzpicture}[@O]
\begin{uncoverenv}<12->
...
/**/}
    ...
    \draw[-Kite] ([xshift=-1.25ex]@.south west) to[out=270,in=0] (pop-insert.east);
\end{uncoverenv}
\end{tikzpicture}
\endAnimateCode
\AnimateCode{onslide={2, 3}, first slide=15}
% scale the markers
\def\ca@Leftmark{\llap{\scalebox{.66}{\@declaredcolor{paletteA}\RawMDRight~}}}
\def\ca@Rightmark{\rlap{\scalebox{.66}{\@declaredcolor{paletteA}\RawMDLeft}}}% currently there is a buffer :D
\def\ca@Botmark{\smash{\clap{\scalebox{.66}{\@declaredcolor{paletteA}\RawMDUp}}}}% currently there is a buffer :D
\begin{tikzpicture}[@O]% allow animation to function by separate
\begin{uncoverenv}<14->% guards, so they no not mess line caps
    \node[below left,xshift=-5mm,fill=btdm@background,rounded corners=1pt,scale=.66] (@) at(current page.east) {\begin{minted}[firstnumber=23,morekeywords={[3]{Stack}}]{java}
...
/**/}
    ...
    \color{gray}
    \only<13->{\color{codeouthl!50!btdm@background}}
    \draw[-Kite] ([xshift=-1.25ex]@.south west) to[out=230,in=0] ([yshift=-1.25mm]pop-insert.east);
\end{uncoverenv}
\end{tikzpicture}
\endAnimateCode
\end{frame}
