...
\newcommand<>\Tunderbrace[2]{\setbox0=\hbox{#1}$\only#3{\expandafter\underbrace}{\copy0}\only#3{_{#2}}$}
\newcommand<>\u[2]{\Tunderbrace#3{#1}{\smash{\clap{\solGet{keywordB}{\scriptsize\HStrut#2}}}}}
\newcommand<>\Toverbrace[2]{\setbox0=\hbox{#1}$\only#3{\expandafter\overbrace}{\copy0}\only#3{^{#2}}$}
\newcommand<>\o[2]{\Toverbrace#3{\text{#1}}{\only#3{\clap{\text{\solGet{keywordB}{\scriptsize\HStrut#2}}}}}}
\begin{frame}[b]{Klammeraffenfreude}
\begin{enumerate}
    \itemsep14pt
    \item<2-> Wir klammern\only<11->{~und erhalten den Typ \bjava{int}}:\smallskip
%{\tracingall
\begin{center}
...
\end{center}
%}
    \item<12-> Wir klammern erneut\only<18->{~und erhalten den Typ \bjava{boolean}}:\smallskip
