...
\begin{itemize}[<+(1)->]
    \item Wir haben dies bereits letzte Woche analysiert, deswegen hier ein wenig schneller.\smallskip
\end{itemize}\lstfs{9}
\begin{plainjava}[lineskip=3pt]
!*\onslide<5->*!int n = Integer.parseInt(args[0]);

!*\onslide<6->*!for (int i = n; i >= 1; i--) {
!*\onslide<7->*!    System.out.println(i); // oder System.out.print(...)
!*\onslide<6->*!}

!*\onslide<12->*!if(n >= 1) {
!*\onslide<8->*!    int j = 1;
!*\onslide<9->*!    do {
!*\onslide<11->*!        if (j % 2 == 1) // ungerade
!*\onslide<11->*!            System.out.println(j);
!*\onslide<10->*!        j = j + 1;
!*\onslide<9->*!    } while (j <= n);
!*\onslide<12->*!}
\end{plainjava}
...
\begin{itemize}[<+(1)->]
    \item Abseits der Datentypentscheidung ist die Java-Syntax das größte Hindernis:\lstfs{10}
\begin{plainjava}
...
!*\onslide<10->*!}
!*\onslide<14->*!!*\KN{10}*!System.out.println(quersumme);
\end{plainjava}
\end{itemize}
\begin{tikzpicture}[overlay,remember picture]
...
!*\onslide<6->*!    System.out.println(y);
!*\onslide<1->*!}
\end{plainjava}
\begin{tikzpicture}[overlay,remember picture]
    \node[below left,yshift=-1.5cm,T] (@) at(current page.north east) {Eingabe: \(k \in \N\), \(k > 1\) und \(x \in \R\), \(x > 0\)~~~};
...
\subsection{Aufgabe 2}
\begin{frame}{Höhere Dimensionen}

\end{frame}

\subsection{Aufgabe 3}
\begin{frame}{Die Welt der Methoden}
...
\end{tikzpicture}\begin{tikzpicture}[scale=1.85]
    ...
    \only<4->{\pingu[devil-horns,right eye devil, left eye angry, devil wings,wings wave,feet simple]}
\end{tikzpicture}}
