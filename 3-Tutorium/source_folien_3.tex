...
\begin{frame}[fragile,c]{Kurzwiederholung}
    \begin{itemize}[<+(1)->]
        ...
        \item Manche von Javas primitiven Datentypen können implizit konvertiert werden \begin{itemize}
            \item \bjava{byte}~\k~\bjava{short}~\k~\bjava{int}~\k~\bjava{long}~\k~\bjava{float}~\k~\bjava{double}~~und~~\bjava{char}~\k~\bjava{int}
        \end{itemize}
...
\begin{aufgabe}{Ich bin der Klammeraffen Typ}
    \small\onslide<2->{Drücken Sie im Folgenden die Präzedenzregeln \textit{explizit} durch Klammern aus, so dass die unten stehenden Ausdrücke auch ohne Präzedenzregeln in der gleichen Reihenfolge ausgeführt werden.} \onslide<3->{Geben Sie weiter den Typ des gesamten Ausdrucks an (\bjava{int i}, \bjava{short s} und \bjava{float f}).} \onslide<4->{Werten Sie die Ausdrücke \textit{nicht} aus.}
\begin{enumerate}
    \itemsep8pt
...
\onslide<8->{
    \node[right=4mm,gray] at(@a) {\raisebox{1pt}{\textbullet}};
...
\SetNextSectionText{There is no real ending. It's just the place where you stop the story.\\--- Frank Herbert~\sectiontextlink\link{http://www.sinanvural.com/seksek/inien/tvd/tvd2.htm}{online}}
...
\outro{\vskip6mm\centering\only<2->{\global\dmfootfalse}\begin{tikzpicture}[@O]
