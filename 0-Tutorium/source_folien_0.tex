...
\IfBtdmDarkmode{\def\boxme#1#2{\only<-12|handout:0>{#2}\only<13->{\boxlot{#1!12!btdm@background}{#2}}}}{\def\boxme#1#2{\only<-12|handout:0>{#2}\only<13->{\boxlot{#1}{#2}}}}
...
\begin{tikzpicture}[overlay,remember picture]
    \onslide<+(1)->{\node[above left,yshift=\btdmfootheight+5mm,xshift=-3.5mm,align=left,gray,font=\footnotesize\sffamily\slshape] at (current page.south east) {Die Laufzeit beläuft sich auf:\\% no cause this seems nicer align
        \(\phantom{{}={}}3 + n \cdot \bigl(1 + \max(2 + 1 + 1, \boxme{shadeA}{$2$} + \boxme{lightgray!50!white!70!shadeB}{$1$} + \boxme{lightgray!90!btdm@background}{$2$}) \bigr) + 1\) \\
        \({}={} 4 + n \cdot 6 \in \O(n)\)};}
\end{tikzpicture}
\end{frame}
}
