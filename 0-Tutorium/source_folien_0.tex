...
\begin{aufgabe}{Das habe ich schon gesehen!}
    \onslide<2->{Entwerfen Sie einen Algorithmus, um herauszufinden, ob in einer Zeichenkette \(S\) das Zeichen \T{a}, aber nicht das Zeichen \T{b} vorkommt. Als Ergebnis soll der Algorithmus ausgeben können, ob diese Bedingung zutrifft (\say{Ja}) oder nicht (\say{Nein}).} \onslide<3->{Einige Beispiele\ldots
\begin{itemize}[<+(1)->]
...
\end{itemize}}
    ...
    \onslide<+(1)->{Problemspezifikation}\onslide<+(1)->{, Problemabstraktion}  \onslide<+(1)->{, Algorithmenentwurf}\onslide<+(1)->{, Korrektheitsnachweis bzw. Verifikation}\onslide<+(1)->{und die Aufwandsanalyse}}
    \onslide<1->
\end{aufgabe}
\end{frame}
}
