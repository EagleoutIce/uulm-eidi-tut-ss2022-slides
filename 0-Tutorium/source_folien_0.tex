...
\def\b{\text{\textit{B}}}
% ugly hell
\makeatletter\def\boxlot#1#2{\text{{\@solLstHL{fill=#1,draw=#1}{#2}}}}
\IfBtdmDarkmode{\def\boxme#1#2{\only<-12|handout:0>{\text{#2}}\only<13->{\boxlot{#1!12!btdm@background}{\text{#2}}}}}{\def\boxme#1#2{\only<-12|handout:0>{#2}\only<13->{\boxlot{#1}{#2}}}}
...
\begin{tikzpicture}[overlay,remember picture]
    \onslide<+(1)->{\node[above left,yshift=\btdmfootheight+5mm,xshift=-3.5mm,align=left,gray,font=\footnotesize\sffamily\slshape] at (current page.south east) {Die Laufzeit beläuft sich auf:\\% no cause this seems nicer align
        \upshape\(\phantom{{}={}}3 + n \cdot \bigl(1 + \max(2 + 1 + 1,~~\boxme{shadeA}{$2$} {+} \boxme{lightgray!50!white!70!shadeB}{$1$} {+} \boxme{lightgray!90!btdm@background}{$2$}) \bigr) + 1\) \\
        \({}={} 4 + n \cdot 6 \in \O(n)\)};}
\end{tikzpicture}
\end{frame}
{\def\xs{.75cm}
...
\begin{frame}{Alternativer Ansatz\hfill II}
    \begin{center}
        {\footnotesize\onslide<2->{\color{gray}Given the String \(S = s^0 s^1 s^2\ldots\)} and \onslide<3->{Let \(i \in \N\), \(Res \in \{\times, \checkmark\}\)}}\medskip
    \end{center}
\centering\resizebox{.925\linewidth}!{\begin{tikzpicture}[rounded corners=1pt,mh/.style={minimum height=2\baselineskip+3mm,rounded corners=1pt,align=center,draw},m/.style={gray,font=\small\sffamily\slshape}]
    ...
    \onslide<12->{\node[mh,below=\xs-2mm] (10) at(6.south) {Increment \(i\) by \(1\)};%
        \draw[-Kite] (7) |- (10.5);
        \onslide<19->{\node[below=-.5mm,m] at(10.south) {$\BF1$};}
    }
...
\begin{tikzpicture}[overlay,remember picture]
    ...
    \onslide<21->{\node[above right,yshift=\btdmfootheight,gray,text width=5cm] at(current bounding box.south west) {Je nach Definition zählt das Inkrement oder nicht. Deswegen ist die \(\O\)-Notation interessanter (diese wird später konkretisiert).};}
\end{tikzpicture}
