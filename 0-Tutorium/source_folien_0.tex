...
\vspace*{-.9\baselineskip}\null\hfill\smash{\raisebox{-\height-.4\baselineskip}{\minipage{.55\linewidth}
\itemize
    \item<4-> \f ababababa->Nein\; \info{Enthält ein \T{b}}
    \item<5-> \f aaaaaaaaa->Ja\; \info{Hat \T{a} und kein \T{b}}
    \item<6-> \f cdefg->Nein\; \info{Enthält kein \T{a}}
    \item<7-> \f klmohna->Ja\; \info{Hat \T{a} und kein \T{b}}
\enditemize
\endminipage}}\par\parshape 7
0pt .4125\linewidth
0pt .4125\linewidth
0pt .4125\linewidth
0pt .4125\linewidth
...
\smallskip\par
    \onslide<8->{Führen Sie die Schritte der Algorithmenentwicklung durch:
    \onslide<+(1)->{a)~Problemspezifikation}\onslide<+(1)->{, b)~Problemabstraktion}  \onslide<+(1)->{, c)~Algorithmenentwurf}\onslide<+(1)->{, d)~Korrektheitsnachweis bzw. Verifikation}\onslide<+(1)->{ und die e)~Aufwandsanalyse.}}
    \onslide<1->
\end{aufgabe}
\end{frame}
}
...
\begin{enumerate}[<+(3)->]
    ...
    \begin{description}[Partielle Korrektheit:]
        \itemsep8pt
        \item[Termination:] Zeilen~\(1\),~\(5\)~\&~\(6\) enthalten Elementar-\\operationen, die per Definition \\terminieren. Die Schleife in Zeile~\(2\)\\terminiert, da \(i\) streng monoton ansteigt und damit in endlicher Zeit mit \(i > n\) die Schleife beendet (die Elementaroperationen in Zeilen~\(3\) und~\(4\) terminieren wieder per Definition).
        \item[Partielle Korrektheit:] Wir merken uns mit \a\ und \b, ob der jeweilige Buchstabe gefunden wurde. Wir geben \say{Ja} nur dann aus, wenn wir ein \T{a}, aber kein \T{b} finden. Sonst geben wir \say{Nein} aus.
    \end{description}
\end{enumerate}
\tikzpicture[overlay,remember picture]
    \onslide<2->{\node[below left,xshift=5mm,yshift=-1.25cm,scale=.85,align=center,gray] at(current page.north east) {\copy\presalgobox\\\itshape\small Eine verkürzte Version};}
\endtikzpicture
\end{frame}
\iffull
\MakeThePinguExplainIt[text width=6.65cm,yshift=-2cm]{cap=!hide,halo,glasses=!hide,eyes shiny,heart=shadeA,body type=chubby,right item angle=-45}{Invarianten mögen vielleicht gruselig klingen, sind aber nur Aussagen, die zum Beispiel im Rahmen von Schleifen \say{immer} (also in jedem Durchlauf) gelten.}
\begin{frame}{Verifikation --- Eine kleine Vertiefung}
   \begin{itemize}[<+(1)->]
    \item Der Alternative, \say{weniger schwammige} Beweis der partiellen Korrektheit: \begin{itemize}
        ...
        \item<7-> Anschließend folgt der Beweis durch die Übereinstimmung der Bedingung, sowie der aussagenlogischen Formel.
    \end{itemize}
   \end{itemize}
...
\begin{itemize}
    ...
    \item<4-> Wir machen es textuell: \begin{itemize}
        \itemsep7pt
        \item<5-> Die 1.~Zeile enthält zwei Zuweisungen, die Bedingung in der\\5.~Zeile enthält zwei Vergleiche und in jedem Fall eine Ausgabe. %\\ (als Elementaroperation).
        \item<6-> Die Schleife enthält zwei Vergleiche und im schlechtesten Fall eine Zuweisung (so ist für \(\text{\T{a}} \neq \text{\T{b}}\) höchstens eine beider Bedingungen trifft zu).
    ...
    \item<8-> So erhalten wir im besten (kein \(a\) und kein \(b\)), sowie im schlechtesten Fall (nur \(a\)'s und \(b\)'s): \begin{align*}
        \onslide<9->\text{best-case} &= 2 + n \cdot (2 + 0) + 2 + 1 = 2n + 5 \in \O(n)\\
        \onslide<10->\text{worst-case} &= 2 + n \cdot (2 + 1) + 2 + 1 = 3n + 5 \in \O(n)
    \end{align*}
\end{itemize}
\tikzpicture[overlay,remember picture]
    \onslide<2->{\node[below left,xshift=5mm,yshift=-1.25cm,scale=.85,align=center,gray] at(current page.north east) {\copy\presalgobox\\\itshape\small Eine verkürzte Version};}
...
\begin{frame}[t]{Schweigsame Annahmen}
    \onslide<2->{\color{btdm@text}Für das Programm \say{Zeichenkettenfilter} gelten die Annahmen:
    \begin{enumerate}[<+(1)->]
    ...
    \end{enumerate}}
    \vfill
    \onslide<6->{\color{gray}Ob die 5.~Zeile wirklich immer zwei Vergleiche hat, hängt davon ab, wie das logische \say{und} funktioniert. Wertet es für \say{\(\text{falsch} \land \langle?\rangle\)} die rechte Seite nicht mehr aus, haben wir Falle keines \(a\)'s einen Aufwand von \(2n + 4\).}
    \vfill
\begin{tikzpicture}[overlay,remember picture]
\onslide<6->{\node[above left,xshift=5mm,scale=.8,yshift=\btdmfootheight] at(current page.south east) {\copy\pinguexplainbox};}% copy for animations
\end{tikzpicture}
